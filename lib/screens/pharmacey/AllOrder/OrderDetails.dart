...
class OrderDetailsPage extends StatefulWidget {
  ...
  OrderDetailsPage(this.orderNum, this.items, this.fcm, {Key? key})
      : super(key: key);
  ...
  totalAmountPharm() {
    ...
    for (int i = 0; i < widget.items.length; i++) {
      appGet.priceOrder.value +=
          double.parse(widget.items[i]['medicinePrice']) *
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBars("orderdetails".tr, false, 93, true, 90),
        body: ListView(
                    ...
                    itemBuilder: (context, index) {
                      return Column(mainAxisSize: MainAxisSize.min, children: [
                        OrderDetailsCard(
                          ...
                          widget.items[index]['medicineAvailability']
                              .toString(),
            ...
            Column(
              ...
              children: [
                Container(
                    ...
                    child: TextButton(
                      onPressed: () {
                        // sendTofcm('success'.tr, 'confirmed'.tr, appGet.fcmToken,
                        //     widget.orderNum);
                        EasyLoading.show(status: 'loading'.tr);
                        updateOrderStatus(4, widget.orderNum, '').then((value) {
                          EasyLoading.dismiss();
                ...
                Container(
                  ...
                  child: TextButton(
                    onPressed: () {
                      EasyLoading.show(status: 'loading'.tr);
                      updateOrderStatus(4, widget.orderNum, 'Canceled')
                          .then((value) {
                        EasyLoading.dismiss();
                        EasyLoading.showSuccess('success'.tr);
                        Get.back();
